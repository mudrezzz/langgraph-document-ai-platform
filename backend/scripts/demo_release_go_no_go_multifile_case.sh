...
CASE_ROOT="${BACKEND_ROOT}/examples/cases/release_go_no_go_multifile_case"
CASE_INPUT_DIR="${CASE_ROOT}/input"
REPORT_MARKDOWN="${CASE_ROOT}/output/release_readiness_report.md"
PID_FILE="${BACKEND_ROOT}/.demo_release_go_no_go_multifile_uvicorn_${PORT}.pid"
...
echo "[1/3] Smoke прогон API с multi-file dataset directory (сервер остается поднятым)..."
"${SMOKE_PATH}" \
    --host "${HOST_NAME}" \
    --port "${PORT}" \
    --query "${QUERY}" \
    --case-dataset-id "release_go_no_go_multifile" \
    --case-dataset-dir "${CASE_INPUT_DIR}" \
    --keep-server \
...
echo "[2/3] Сбор evidence и audit summary по задаче ${TASK_ID}..."
...
echo "[3/3] Генерация release readiness отчета..."
...
echo "=== DEMO: Release Go/No-Go Multi-File Case ==="
echo "Input dir: ${CASE_INPUT_DIR}"
